...
# MAGIC %sql
# MAGIC SHOW GRANTS ON EXTERNAL LOCATION covid_reporting_el_raw;
...
# Updated path to point exactly to the extensionless file name
df_raw_population = spark.read.csv(
    "abfss://raw@covidreportingdl2627.dfs.core.windows.net/population/population_by_age_tsv", 
    sep=r'\t', 
    header=True
)
...
df_raw_population_pivot = spark.sql("""
    SELECT 
        country_code, 
        age_group, 
        try_cast(regexp_replace(percentage_2019, '[^0-9.]', '') AS decimal(4,2)) AS percentage_2019 
    FROM raw_population 
...
df_raw_population_pivot.display()
...
# Updated with secure ABFSS path pointing to the lookup container
df_dim_country = spark.read.csv(
    "abfss://lookup@covidreportingdl2627.dfs.core.windows.net/dim_country", 
    sep=r',', 
    header=True
)
...
df_processed_population.write.format("csv") \
    ...
    .option("delimiter", ",") \
    .mode("overwrite") \
    .save("abfss://processed@covidreportingdl2627.dfs.core.windows.net/population")
